...
\scriptsize
\begin{multicols}{3}
\textbf{\underline{x86 machine instruction}}
...
\textbf{pushq} SRC $\rightarrow$ rsp $\leftarrow$ rsp - 8; Mem[rsp] $\leftarrow$ SRC
...
$eq$: ZF is set; $neq$: not ZF; $lt$: SF $\neq$ OF $(SF \wedge \neg OF) \vee (\neg SF \wedge OF)$ ; $ge$: not $IT$ HOLDS $\implies$ $SF \equiv OF$; $le$: $lt$ OR ZF; $gt$: Not $le$
...
\textbf{\underline{LLVM intermediate representation}}
...
5. \textbf{LR grammar \& Shift-reduce Parsing}
...
LR Parser state: Stack of terminals and nonterminals; Unconsumed input is a string of terminals; Current derivation step is stack + input
...
\underline{Constructing DFA for shift-reduce Parser states}
...
G. LR(0) limitations: \textbf{States with reduce actions have a single reduce action} $\implies$ More complex grammar will cause shift/reduce and reduce/reduce conflict $\implies$ Can resolve by using look-ahead symbol
...
the parser knows the entire stack contents and the next input token, cannot decide whether to shift or to reduce (\underline{shift/reduce}) OR cannot decide which reduction to make (\underline{reduce/reduce}) $\rightarrow$ Technically, this grammar are not in LR(k)


\textbf{\underline{Lambda Calculus}}

1. \textbf{Untyped Lambda Calculus Syntax} 
\begin{lstlisting}[language=ml]
    type exp = | Var of var | Fun of var * exp 
    | App of exp * exp
\end{lstlisting}
...
\textbf{\underline{Semantic Analysis - Type Checking}}
...
\textbf{\underline{Code optimization}}
...
$use[n]$, $def[n]$, $in[n]$, $out[n]$

$F_n(out[n]) = use[n] - (out[n] - def[n])$

\textbf{Reaching Definition Analysis}

Domain: set of nodes

$def[a]$ - set of all nodes defined $a$; 

$gen[n] \equiv a = b\ op\ c\ |\ a = load\ b \rightarrow \{n\}$

$kill[n] \equiv a = load\ b\ |\ a = b\ op\ b\ |\ a = f(b_1, \dots, b_n) \rightarrow def[a] - {n}$

$F_n(in[n]) = gen[n] \cup (in[n] - kill[n])$

\textbf{Available Expressions Analysis}

Domain: set of nodes

$use[a]$ set of nodes that used $a$ in there expression

$gen[n] \equiv a = b\ op\ c\ |\ a = load\ b \rightarrow \{n\} - kill[n]$

$kill[n] \equiv a = b\ op\ c \rightarrow uses[a]; a=load\ b \rightarrow uses[a]; store\ b, a \rightarrow uses[[x]],\forall x \text{ may equal a}; a = f(b_1, \dots b_n) \rightarrow uses[a] \cup uses[[x]] \forall x \text{ may equal to b}; f(b_1, \dots, b_n) \rightarrow uses[[x]]$ 
...
\includegraphics[width=7cm, height=2.5cm]{images/worklist_algo.png}
...
\textbf{Forward dataflow analysis}

- control flow constraint: $in[n] = \sqcap_{n'\in pred[n]}out[n']$

- transfer constraint: $out[n] = F_n(in[n])$

\textbf{Backward dataflow analysis}

- control flow constraint: $out[n] = \sqcap_{n'\in succ[n]}in[n']$

- transfer constraint: $in[n] = F_n(out[n])$

\includegraphics[width=8cm, height=3.5cm]{images/generic_iterative_analysis.png}
