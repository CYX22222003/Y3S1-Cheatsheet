...
Basic form: SELECT [DISTINCT] target-list FROM relation-list [WHERE conditions]
...
1) IN/ NOT IN: expr IN/NOT IN (subquery): subquery must return exactly one column; IN returns true if expr matches any row; NOT IN the other way round;
2) ANY/SOME: 
...
\underline{entity}: objects; \underline{entity set}: collection of objects; \underline{attribute}: information describing an entity; \underline{key attribute}: uniquely identify each entity; \underline{derived attribute}: derived from other attributes; composite attributes, multivalued attribute; Union of all PKs of its participating set is the \underline{superkey}

\underline{relationship and relationship set}: collection of tuples of object of same types, representing association among several attributes. $E_1$, $E_2$, ... $E_n$ participates in relationship set R. May contains descriptive attributes
...
\includegraphics[width=4cm, height=3.5cm]{images/many_to_many.png}
\includegraphics[width=4cm, height=3.5cm]{images/many_to_one.png}
\includegraphics[width=4cm, height=3.55cm]{images/many_to_one2.png}
\includegraphics[width=4cm, height=3.5cm]{images/one_to_one.png}
\includegraphics[width=4cm, height=3.5cm]{images/weak_entity_set.png}
Use NOT NULL to ensure participation constraint
...
\textbf{Projection}: $\pi_l(R)$ projects all attributes of a relation specified in list $l$, \underline{duplicates tuples are removed from output} as relation is a set
...
\textbf{Declaration}: DECLARE var\_name, var\_type;

\textbf{IF-THEN-ELSE}: block specify with THEN and ends with END IF

\textbf{loop}: 

$LOOP\ EXIT\ [label]\ WHEN\ [condition]\ [stmt]\ END\ LOOP;$

$\ WHILE\ [cond]\ LOOP\ [stmt]\ END\ LOOP;$

$\ FOR\ [var_name]\ IN\ [LOWER...UPPER]\ LOOP\ [stmt]\ END\ LOOP$
...
Cursor: Declare $\rightarrow$ Open $\rightarrow$ Fetch $\rightarrow$ Exit when not found $\rightarrow$ Close
...
\verb|BEFORE| trigger is executed before the event. May modify/cancel the event by using \textit{return value}.

\verb|AFTER| trigger is executed after the event. The event already occurred, cancellation done using \verb|EXCEPTION|.

\textbf{Events}: INSERT, DELETE, UPDATE

\textbf{Variables}

\textbf{NEW (OLD)} refers to the what the row like \textbf{after (before)} the operation

\verb|INSERT|: new value (\verb|NEW|)

\verb|UPDATE|: old value (\verb|OLD|) \& new value (\verb|NEW|)

\verb|DELETE|: old value (\verb|OLD|)

\textbf{Return Values}

1) BEFORE INSERT/UPDATE/DELETE $\rightarrow$ NULL tuple: noting; NON-NULL tuple t: t will be inserted or updated but no effect on delete

2) AFTER INSERT/UPDATE/DELETE $\rightarrow$ Return value has no effect completely
...
\textbf{Granularity: FOR EACH ROW, FOR EACH STATEMENT}

Some checks may only need to be done once regardless of how many rows are affected.

Return value has no effect on the subsequent operations. Cancellation has to be done using EXCEPTION for statement-level.
...
\textbf{Deferment}: check only after end of transaction, can only be AFTER and FOR EACH ROW trigger

\begin{lstlisting}[language = sql]
CREATE CONSTRAINT TRIGGER <trigger\_name> 
<trigger\_timing> <trigger\_event> ON <trigger\_table>
[ DEFERRABLE INITIALLY [ DEFERRED | IMMEDIATE ] ]
FOR EACH <trigger\_granularity>
[ WHEN <trigger\_condition> ]
EXECUTE FUNCTION <trigger\_function\_name>(); 
\end{lstlisting}

\textbf{Activation order}

BEFORE stmt $\rightarrow$ BEFORE row $\rightarrow$ EVENT for row $\rightarrow$ AFTER row $\rightarrow$ AFTER stmt
...
\textbf{\underline{ERD mapping with FDs}}
...
4. \textbf{How to find}: always looking for the potential PK from the smallest cardinality; if an attribute does not appear in any RHS, it must be a PK; when growing from small cardinality, choose attributes that is not in the current attribute closure
...
Find all possible subset $R'' \subseteq R'$, calculate their attribute closure $S$, eliminate elements not in $R'$ and make them nontrivial. Can return the minimal cover only
...
\underline{\textbf{How to quickly determine?}} Look at all relation in the $\Sigma$ $\Rightarrow$ if LHS and RHS appear in the same, confirmed entailed $R_i$ $\Rightarrow$ if they appear on different sides, check LHS $\rightarrow$ (Common attributes) and (Common attributes) $\rightarrow$ RHS.
